...
def check_data_exist(email:str):
    file_path = f'data/game_data/player/{email.split("@")[0]}.json'
...
def data_load():
    ...
    with open(f'data/game_data/player/{email_reader().split("@")[0]}.json', 'r') as file:
        data = json.load(file)
...
def insert_data(data):
    ...
    with open(f'data/game_data/player/{email_reader().split("@")[0]}.json', 'w') as file:
        json.dump(data, file, indent=4)  
